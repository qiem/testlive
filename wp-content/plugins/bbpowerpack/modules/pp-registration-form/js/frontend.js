(function($) {
	...
	PPRegistrationForm = function( settings ) {
	...
	PPRegistrationForm.prototype = {
		...
		_getFormData: function() {
			var formData = new FormData( $(this.nodeClass + ' .pp-registration-form')[0] );
		...
		_submit: function(e) {
			...
			var theForm			= this.form,
				submit	  		= $(this.nodeClass + ' .pp-button'),
			...
			if ( ! this.isValid ) {
				...
				formData.append( 'action', 'pp_register_user' );
		...
		_submitComplete: function( response ) {
			...
			if ( typeof response.success !== 'undefined' && response.success === true ) {
				$( this.nodeClass + ' .pp-rf-failed-error' ).fadeOut();
				if ( 'undefined' !== typeof response.data.redirect_url ) {
					window.location.href = response.data.redirect_url;
				} else if ( noMessage.length > 0 ) {
					noMessage.fadeIn();
				} else {
					$( this.nodeClass + ' .pp-registration-form' ).hide();
					$( this.nodeClass + ' .pp-after-submit-action.pp-success-custom-message' ).fadeIn();
				}
				...
				if ( typeof response.data.message !== 'undefined' ) {
					...
					if ( field && field.length > 0 ) {
						...
						$(this.nodeClass + ' .pp-rf-failed-error').html( message );
						$(this.nodeClass + ' .pp-rf-failed-error').fadeIn();
					}
				} else {
					$(this.nodeClass + ' .pp-rf-failed-error').fadeIn();
				}
